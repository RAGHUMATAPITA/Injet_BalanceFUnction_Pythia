...
void Tree_Analyzer_NewNew(TString input_file, int itxtoutFile, TString out_directory, TString name_of_roorFile)
{
  ...
  for(int itxt = 0; itxt < itxtoutFile; itxt++)
      ...
      for(int iev = 0; iev < 200; iev++)
	{
	  ...
	  // push back event quatities (pthatweught, Subprocesscode, etc) before closing event loop
	  pthatw_vec_1D.push_back(pthatweight);
	  processID_vec_1D.push_back(Subprocesscode);

	  //fill event histograms
	  hntrkTotal->Fill(ntrkTotal);
	  hpthat->Fill(pthat);
	  hpthatw->Fill(pthatweight);
	  hsubprocessid->Fill(Subprocesscode);
	  hnjets->Fill(njets);
	} // event loop end
  ...
  // output file name
  std::string outfilename = Form("%s/%s_Outfile", out_directory.Data(), name_of_roorFile.Data());
